###############################################################################
# fuel_proxy/models/_resume_after_ppml.R
#
# PURPOSE
#   Resume execute_models.R after the PPML proxy loop completed successfully
#   but append_metrics_log() failed (e.g. CSV was open).
#
#   This script:
#     1) Re-creates the CSV from the intact .rds metrics log
#     2) Runs the full hurdle pipeline (step 1, step 2, triple evaluation, LOSOCV)
#     3) Runs execute_descriptives.R
#
# USAGE
#   source("fuel_proxy/models/_resume_after_ppml.R")
###############################################################################
...
# =====================================================================
# FIX: Re-export CSV from the intact .rds
# =====================================================================
cat("\n", strrep("=", 70), "\n")
cat("  FIXING: Re-exporting model_performance_metrics.csv from .rds\n")
cat(strrep("=", 70), "\n\n")

if (file.exists(METRICS_PATH_RDS)) {
  existing_metrics <- readRDS(METRICS_PATH_RDS)
  data.table::fwrite(as.data.table(existing_metrics), METRICS_PATH_CSV)
  cat(sprintf("  CSV re-exported: %d rows -> %s\n", nrow(existing_metrics), METRICS_PATH_CSV))
} else {
  stop("METRICS_PATH_RDS not found: ", METRICS_PATH_RDS,
       "\nCannot resume — the PPML metrics .rds must exist.")
}


# =====================================================================
# HURDLE MODEL (from execute_models.R lines 224-658)
# =====================================================================
...
# -----------------------
cat("\n", strrep("=", 70), "\n")
cat("  HURDLE: Precomputing Step 1 (extensive margin)\n")
cat(strrep("=", 70), "\n\n")
...
# -----------------------
cat("\n", strrep("=", 70), "\n")
cat("  HURDLE: Precomputing Step 2 (intensive margin)\n")
cat(strrep("=", 70), "\n\n")
...
# -----------------------
cat("\n", strrep("=", 70), "\n")
cat("  HURDLE: Evaluating top-K triples\n")
cat(strrep("=", 70), "\n\n")
...
# =====================================================================
cat("\n", strrep("#", 70), "\n")
cat("  STAGE: DESCRIPTIVES\n")
cat(strrep("#", 70), "\n\n")

source(file.path(REPO_DIR, "fuel_proxy", "descriptives", "execute_descriptives.R"),
       local = FALSE)
