...
#
# PURPOSE
#   Identical to build_loso_proxy.R but with asinh(y) on LHS.
...
#     Contains: loso_proxy_panel_asinh, loso_sector_map_asinh,
#               loso_fold_diagnostics_asinh, loso_supplier_overlap_asinh, syt_asinh
#
...
loso_sector_map_asinh <- data.frame(
  nace2d = all_sectors,
...
sector_fy <- lhs %>%
  ...
  summarise(n_fy = n(), n_emitter_fy = sum(emit), n_firms = n_distinct(vat),
            .groups = "drop")
loso_sector_map_asinh <- loso_sector_map_asinh %>%
  left_join(sector_fy, by = c("nace2d" = "primary_nace2d"))

cat("\n-- Sector summary --\n")
print(loso_sector_map_asinh)

lhs <- lhs %>%
  left_join(loso_sector_map_asinh %>% select(nace2d, fold_k),
...
# =============================================================================
cat("=== LEAVE-ONE-SECTOR-OUT CV (asinh LHS):", S, "sectors x elastic net ===\n\n")
...
for (k in seq_len(S)) {
  ...
  if (n_selected_pos_k > 0) {
    ...
    proxy_k <- b2b_heldout_k %>%
      ...
      summarise(loso_proxy_asinh = sum(coef * asinh(corr_sales_ij), na.rm = TRUE),
                .groups = "drop") %>%
      rename(vat = vat_j_ano)
    rm(b2b_heldout_k)
  } else {
    proxy_k <- data.frame(vat = character(0), year = integer(0),
                          loso_proxy_asinh = numeric(0))
  }
  ...
  if (nrow(coef_lookup_k) > 0) {
    ...
    proxy_all_k <- b2b_heldout_all_k %>%
      ...
      group_by(vat_j_ano, year) %>%
      summarise(loso_proxy_all_asinh = sum(coef * asinh(corr_sales_ij), na.rm = TRUE),
                .groups = "drop") %>%
      rename(vat = vat_j_ano)
    rm(b2b_heldout_all_k)
  } else {
    proxy_all_k <- data.frame(vat = character(0), year = integer(0),
                              loso_proxy_all_asinh = numeric(0))
  }
  ...
  test_proxy_k <- test_lhs %>%
    ...
    mutate(
      loso_proxy_asinh     = coalesce(loso_proxy_asinh, 0),
      loso_proxy_all_asinh = coalesce(loso_proxy_all_asinh, 0)
    )

  proxy_pieces[[k]]     <- test_proxy_k %>% select(vat, year, loso_proxy_asinh)
  proxy_pieces_all[[k]] <- test_proxy_k %>% select(vat, year, loso_proxy_all_asinh)
  ...
  share_test_proxy_gt0 <- if (n_test_fy > 0) mean(test_proxy_k$loso_proxy_asinh > 0) else NA_real_
  share_test_emitter_proxy_gt0 <- if (n_test_emitter_fy > 0) {
    mean(test_proxy_k$loso_proxy_asinh[test_proxy_k$emit == 1L] > 0)
  } else {
    NA_real_
  }

  # Proxy quality on held-out fold (against original y)
  cor_proxy_y_k <- if (sum(test_proxy_k$y > 0 & test_proxy_k$loso_proxy_asinh > 0) > 5) {
    cor(test_proxy_k$y, test_proxy_k$loso_proxy_asinh, use = "complete.obs")
  } else {
    NA_real_
  }

  auc_emit_k <- compute_auc(test_proxy_k$emit, test_proxy_k$loso_proxy_asinh)

  cat("  Proxy > 0:", sum(test_proxy_k$loso_proxy_asinh > 0), "/", n_test_fy, "\n")
...
loso_proxy_panel_asinh <- lhs %>%
  ...
  select(vat, year, nace2d, y, emit, log_revenue, euets,
         primary_nace2d, fold_k, loso_proxy_asinh, loso_proxy_all_asinh)

loso_fold_diagnostics_asinh <- bind_rows(diag_rows)

cat("LOSO proxy panel (asinh LHS):", nrow(loso_proxy_panel_asinh), "rows\n")
cat("  loso_proxy_asinh > 0:", sum(loso_proxy_panel_asinh$loso_proxy_asinh > 0),
    sprintf("(%.1f%%)\n", 100 * mean(loso_proxy_panel_asinh$loso_proxy_asinh > 0)))
...
loso_supplier_overlap_asinh <- list(
...
# =============================================================================
cat("\n-- Fold diagnostics --\n")
print(loso_fold_diagnostics_asinh[, c("fold_k", "held_out_sector", "n_test_firms",
...
# STEP 11: Save
# =============================================================================
OUT_PATH <- file.path(PROC_DATA, "loso_proxy_asinh.RData")
save(loso_proxy_panel_asinh, loso_sector_map_asinh, loso_fold_diagnostics_asinh,
     loso_supplier_overlap_asinh, syt_asinh,
     file = OUT_PATH)

cat("\n==============================================\n")
cat("Saved to:", OUT_PATH, "\n")
cat("  loso_proxy_panel_asinh:       ", nrow(loso_proxy_panel_asinh), "rows\n")
cat("  loso_sector_map_asinh:        ", nrow(loso_sector_map_asinh), "rows\n")
cat("  loso_fold_diagnostics_asinh:  ", nrow(loso_fold_diagnostics_asinh), "rows\n")
cat("  loso_supplier_overlap_asinh:   list with", length(loso_supplier_overlap_asinh), "elements\n")
