...
#   concern that the pooled elastic net simply captures suppliers who sell to
#   high-emitting firms (cross-sectional sorting), rather than fuel suppliers
#   whose sales causally relate to emissions.
#
#   Three regressions (LHS = asinh(emissions)):
...
# ── Build fuel proxy from pooled-identified suppliers ────────────────────────
...
proxy_df <- b2b %>%
  filter(vat_i_ano %in% suppliers_pooled, vat_j_ano %in% lhs$vat, year >= 2005) %>%
...
# ── Variance decomposition of fuel proxy ─────────────────────────────────────
cat("\n", strrep("=", 60), "\n")
...
# How many firms have time variation in proxy?
...
# ══════════════════════════════════════════════════════════════════════════════
# REGRESSION 1: Pooled OLS with sector + year FE
# ══════════════════════════════════════════════════════════════════════════════
cat("\n", strrep("=", 60), "\n")
...
# ══════════════════════════════════════════════════════════════════════════════
# SUMMARY: Coefficient comparison
# ══════════════════════════════════════════════════════════════════════════════
cat("\n\n", strrep("=", 60), "\n")
cat("  SUMMARY: asinh(proxy) coefficient across specifications\n")
cat(strrep("=", 60), "\n\n")
...
if (nrow(summary_table) >= 2) {
  pooled_coef <- summary_table$coef[1]
  fe_coef     <- summary_table$coef[2]
